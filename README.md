...
```bash
./search ../../Examples/example.xml
```

#### Работа алгоритма Дейкстры

После запуска данного алгоритма в окне терминала будет выведена информация о результате поиска, а в директории с входным файлом появится выходной файл:

* `example_log_Dijkstra.xml` &mdash; результат работы алгоритма Дейкстры

Этот файл можно визуализировать как описано [тут](#Visualise). 

Пример:

<img src=".\Images\DijkstraRun.jpg" align="left" width="550" height="288" /><br><br><br><br><br><br><br><br><br><br><br><br>

#### Работа A*

После запуска данного алгоритма в окне терминала будет выведена информация о результате поиска, а в директории с входным файлом появится выходной файл:

* `example_log_AStar.xml` &mdash; результат работы алгоритма A*

Этот файл можно визуализировать как описано [тут](#Visualise). 

Пример:

<img src=".\Images\AStarRun.jpg" align="left" width="550" height="288" /><br><br><br><br><br><br><br><br><br><br><br><br>
...
* `example_log_TMP.xml` &mdash; файл с построенным LPA* путём, необходимый для визуализации карты во время взаимодействия с программой.

Эти файлы можно визуализировать как описано [тут](#Visualise). 

Результат первого поиска:

<img src="./Images/LPAStarFirstRun.jpg" align="left" width="518" height="567" /><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
...
<img src=".\Images\LPAStarSecondRun.jpg" align="left" width="518" height="567" /><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>

#### Работа D* Lite

После запуска на консоль будет выведен результат работы двух алгоритмов: A* и D* Lite. Затем в директории с входным файлом появятся файлы:

* `example_log_AStar.xm` &mdash; результат работы алгоритма A*
* `example_log_DStarLite.xml` &mdash; результат работы алгоритма D* Lite
* `example_log_TMP.xml` &mdash; файл с построенным D* Lite путём, необходимый для визуализации карты во время взаимодействия с программой.

Эти файлы можно визуализировать как описано [тут](#Visualise). 

Результат первого поиска:

<img src=".\Images\DStarLiteFirstFind.jpg" align="left" width="518" height="567" /><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>

После ознакомления с результатамии можно продолжить выполнение программы, которое будет заключаться в добавлении на карту непроходимой клетки. 
...
В графическом окне откроется карта, на которой голубым отмечен найденный путь. Необходимо левой кнопкой мыши нажать на одну из клеток, которую Вы желайте сделать непроходимой. Сразу после нажатия препятствие добавится в исходный входной файл, основная программа обработает его. После этого по нажатии кнопки Enter в терминале с основной программой можно передвигать робота по построенному пути. Для обновления картинок в окнах визуализаторов файлов необходимо на любую из клеток каждого из файлов нажать правой кнопкой мыши. В тот момент, когда робот обнаружит добавленное препятствие на своём пути препятствие, будут перезапущены оба алгоритма, при этом A* будет работать "заново", а D* Lite использует посчитанные на предыдущей итерации значения. После завершения поиска в окно терминала с запущенной основной программой будут выведены новые логи, выходные файлы перезапишутся. После обновления визуализаторов, в окне D* Lite розовым будут отмечены клетки, с которыми и только с которыми (не считая голубых) алгоритм взаимодействовал при повторном запуске, при этом жёлтые клетки остаются нетронутыми с самого первого запуска. Далее при новых нажатиях на Enter робот продолжит движение до финиша. 

Результат перепланирования траектории:

<img src=".\Images\DStarLiteSecondFind.jpg" align="left" width="518" height="567" /><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
...
##### Поддерживаемые опции поиска

* `searchtype` &mdash; алгоритм поиска
  * `dijkstra` &mdash; алгоритм Дейкстры
  * `astar` &mdash; A*
  * `lpastar` &mdash; LPA*
  * `dstarlite` &mdash; D* Lite

* `metrictype` &mdash; эвристика для оценки расстояния (Для Дейкстры это всегда 0)
...
* `hweight` &mdash; вес эвристики при подсчете f-вершины (число >= 1).
  ...
  * \>1 &mdash; взвешенный A* (WA*) 
  ...
  >
  > Для LPA* и D* Lite разрешён лишь hweight = 1
...
## <a name="Visualise"></a>Визуализация выходного файла

Все выходные файлы можно виуализировать при помощи Python3 скриптов. Для этого откройте новую вкладку терминала в корневой директории проекта `.../PathPlanningProject/`
...
можно будет увидеть карту соответствующего файла (в примере A*), где:
...
* розовые клетки &mdash; проходимые клетки, которые были обработаны алгоритмами LPA* или D* Lite при перепланировании траектории
